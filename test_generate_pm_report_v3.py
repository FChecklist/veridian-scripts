...
def test_end_to_end_smoke_run(tmp_path, monkeypatch):
    ...
    for expected in [
        ...
        "16. TRACKED PR MERGE STATE",
        "17. RECENT OWNER-DISPATCHED UMR STATUS",
...
def test_render_report_text_survives_per_metric_insufficient_data(monkeypatch):
    ...
    assert "16. TRACKED PR MERGE STATE" in text
    assert "17. RECENT OWNER-DISPATCHED UMR STATUS" in text
...
# --- Section 16 (this task's own real ask): real tracked-PR merge state
...
# --- Section 17 (this task's own real ask): real per-row status for
