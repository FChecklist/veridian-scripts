...
#
# RESTORED 2026-08-02 (Sentinel finding): this constant, reconcile_stale_
# heartbeats(), _unit_exit_terminal_status(), and the --reconcile-stale CLI
# arg were silently dropped by a deploy after 2026-07-30 (the deploy appears
# to have been built from a stale pre-Stage-3 copy of this file -- see
# resource_governor.py.bak-predeploy-20260730-040659). The tick loop kept
# invoking `--reconcile-stale` every 30s the whole time; every call failed
# with argparse's "unrecognized arguments" (7946 occurrences logged in
# resource_governor_tick.log by the time this was caught), so the
# reconciliation sweep was silently dead the whole time, not just slow.
...
# ---------------------------------------------------------------------------
# Heartbeat reconciliation sweep (Stage 3, 2026-07-29; restored 2026-08-02
# after a post-2026-07-30 deploy silently dropped it -- see
# HEARTBEAT_STALE_TTL_SECONDS' comment above for the incident detail)
# ---------------------------------------------------------------------------
