#!/usr/bin/env python3
"""
phase-continuation-tick.py -- task-20260726-210339 consolidation: refactor of
auto_phase_continuation.py (INS-20260724-113032-8032 mechanism 2). Plan-scan,
merged-PR cross-check, next-phase selection, and the tier2 self-report backfill
sweep are UNCHANGED from the original -- see that script's own extensive
docstring (kept below, verbatim) for why each of those exists.

Two real changes for this consolidation:
  1. The one real spawn call site (task-gateway.py start, inside dispatch())
     now acquires dispatch_core.acquire_dispatch_lock() and checks
     has_free_slot() first -- auto_phase_continuation.py previously dispatched
     every ready, not-yet-dispatched phase it found with NO concurrency
     accounting at all, the third of the 3 independent spawn paths this task's
     SPEC names as the real root cause of the 2026-07-26 OOM-kill (the other
     two are dispatch-tick.py's supervisor-sweep/queue-dispatcher/module-queue
     sections). A phase that can't get a free slot this tick is simply left
     not-yet-dispatched -- already_dispatched() only recognizes a REAL
     task/PR, so it will be picked up again, for real, on the next tick.
  2. Every run now writes ai-os/PHASE_READY_CACHE.json, a small timestamped
     JSON file listing every ready-to-advance-but-not-yet-dispatched phase
     found this run -- same 3 fields (initiative/next_phase/generated_title)
     veridian_status_monitor.py's old scan_phases_ready() used to extract from
     a full `auto_phase_continuation.py --dry-run` subprocess re-run. That
     redundant re-scan (this script's ENTIRE plan-scan + per-phase `gh pr
     list`/`git show master:...` cross-reference work, duplicated purely to
     populate one read-only status field, every 10-minute status-remediation
     tick) is what status-remediation-tick.py replaces by reading this cache
     file instead -- see this task's SCOPE item 5.

--- original auto_phase_continuation.py docstring, unchanged below ---
...
PLAN_DIRS = [
    AI_OS,
    os.environ.get("VERIDIAN_REPO_AI_OS_DIR", f"{VERIDIAN_ROOT}/repos/claude-control/ai-os"),
]
...
def master_shows_phase_done(plan_filename, phase_id, repo_path=None):
    ...
    repo_path = repo_path or f"{VERIDIAN_ROOT}/repos/claude-control"
...
{deps_note}. This task was auto-dispatched by scripts/phase-continuation-tick.py (formerly
auto_phase_continuation.py) per instruction INS-20260724-113032-8032, which exists specifically because
this same initiative's phased dispatch had previously stalled after an earlier phase with nothing to
continue it automatically -- do not stop after this phase either without updating the plan file per
EXPECTED_OUTPUT below, so the next cron tick can find real next-phase work if one exists.
...
a future run of phase-continuation-tick.py can correctly detect this phase as done and move on to whatever
depends on it.
...
def write_phase_plan_sidecar(task_id, plan_filename, phase_id, initiative_name):
    ...
    task_dir = os.path.join(dispatch_core.TASKS_DIR, task_id)
    try:
...
def dispatch(prompt_text, title, repo=DEFAULT_REPO, plan_filename=None, phase_id=None, initiative_name=None):
    ...
    # Real spawn call site: gated by the shared concurrency lock/cap, same as
    # every other real spawn call site in dispatch-tick.py. `submit` above is
    # bookkeeping only (registers an instruction, no worker spawned) -- `start`
    # is what actually calls veridian-task.py create + systemctl start
    ...
    if dispatched_task_id:
        dispatch_core.record_dispatch_event(
            task_id=dispatched_task_id, dispatched_by="phase-continuation-tick.py",
            source_queue_or_plan=plan_filename or "unknown_plan",
            worker_unit=f"veridian-worker@{dispatched_task_id}.service",
            extra={"phase_id": phase_id, "initiative": initiative_name})
...
# ---------------------------------------------------------------------------
# PHASE_READY_CACHE.json (SCOPE item 4) -- read by status-remediation-tick.py
# instead of that script re-invoking this one's full discovery every tick.
# ---------------------------------------------------------------------------

def build_phase_ready_cache(report):
    ready = []
    for entry in report:
        if entry.get("next_phase") is not None and not entry.get("already_dispatched"):
            ready.append({
...
def write_phase_ready_cache(report):
    payload = {
        "generated_at": now_iso(),
        "generator": "scripts/phase-continuation-tick.py",
        "phases_ready_to_advance": build_phase_ready_cache(report),
    }
    tmp = PHASE_READY_CACHE_PATH + ".tmp"
    os.makedirs(os.path.dirname(PHASE_READY_CACHE_PATH), exist_ok=True)
    with open(tmp, "w") as f:
        json.dump(payload, f, indent=2, default=str)
    os.replace(tmp, PHASE_READY_CACHE_PATH)
    return PHASE_READY_CACHE_PATH
...
def main():
    ...
    cache_path = write_phase_ready_cache(report)

    dispatch_core.record_tick(
        "phase-continuation-tick", status="ok", dispatched_this_tick=dispatched_count,
        extra={"phase_ready_cache": cache_path})

    print(json.dumps({"dry_run": args.dry_run, "tier2_self_report_sweep": sweep_result,
                       "initiatives": report, "phase_ready_cache": cache_path}, indent=2, default=str))
