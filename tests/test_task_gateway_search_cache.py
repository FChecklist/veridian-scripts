...
def _submit(tg, text, session_id):
    # task-20260814-180459 (file-attachment intake), merged into main after
    # this test was written, added a real --attach arg that cmd_submit now
    # reads unconditionally -- attach=None matches its real argparse default
    # for a plain-text-only submission (there is no CLI here to populate it).
    args = Namespace(text=text, source="ai_agent", session_id=session_id, tier=None, attach=None)
    buf = io.StringIO()
