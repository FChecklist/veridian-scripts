...
- [x] category_index=7 (regression testing): **pass**, real -- defined deterministically as "does
      ...
      row back directly from the DB. Branch: feat/gtm-checks-regression-testing.

## Remaining (this task, so far)
- [ ] category_index=9 (performance testing), 15/16 (multi tenant / role permission, expected
      blocked), 24 (lighthouse audit), 25 (production readiness synthesis, built last) -- not yet
      reached in this session; see task-level Remaining notes on the final branch once built, or
      the still-open `child-umr-ocid020-gtm-remaining-8-category-scripts` UMR row if this session
      ends before they are.
- [ ] Both PRs opened so far (#79 and this branch's) need `supervisor-sweep.sh` pickup and a real
      independent audit verdict before merge, same standing discipline as every other open PR in
      this repo.
