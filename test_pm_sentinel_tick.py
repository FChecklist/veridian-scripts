...
def _gen_umr_id():
    """Real UMR-id-shaped test fixture id (UMR-YYYYMMDD-HHMMSS-xxxx) -- unlike
    the fixed UMR-TESTFIX-... prefix the older tests above use, the new
    UMR-20260813-150004 tests below need ids that actually match
    superboss-register.py's own _TARGET_ID_UMR_RE (\\bUMR-\\d{8}-\\d{6}-
    [0-9a-fA-F]{4}\\b), since that regex is exactly what is under test."""
    n = uuid.uuid4().int
    return f"UMR-20260101-{n % 1000000:06d}-{uuid.uuid4().hex[:4]}"


def _insert_row(copy_path, umr_id, task_identity, status, reason, title="", prompt="",
                 repo="claude-control"):
    """Same real-schema INSERT _seeded_copy() does, extended with a real
    inputs_json (title/prompt/repo) -- needed for the UMR-20260813-150004
    tests below, which check real target-identifier extraction and
    RCA_DEPTH-marker parsing against inputs_json.title/prompt, neither of
    which the original _seeded_copy() rows populate (they only needed
    status/reason)."""
    conn = sqlite3.connect(copy_path)
    conn.execute(
        "INSERT INTO umr_tasks (umr_id, task_identity, ts_submitted, tier, "
        "status, source_trigger, task_kind, reason, inputs_json) VALUES "
        "(?,?,datetime('now'),1,?,'test_seed','veridian_task_create',?,?)",
        (umr_id, task_identity, status, reason,
...
class PmSentinelTickRcaTargetDedupTest(unittest.TestCase):
    """Real regression test for UMR-20260813-150004 (self-amplifying RCA
    cascade), item B/E: the real incident this addendum fixes --
    UMR-20260813-091810-5045 and UMR-20260813-124141-7641, both titled
    "RCA: UMR-20260813-060311-6eea killed", the second dispatched ~3h15m
    after the first had already reached status=completed_unmerged with a
    real primary deliverable produced. A killed row whose RCA already
    exists in ANY of queued/dispatched/running/completed/completed_unmerged
    must not get a second RCA dispatched for it -- proven here with a real
    before/after --query-umr-equivalent row count over a real, isolated DB
    copy: zero new rows."""

    def setUp(self):
        self.TARGET_UMR = _gen_umr_id()
        self.EXISTING_RCA_UMR = _gen_umr_id()
        self.tmpdir = tempfile.mkdtemp(prefix="pm_sentinel_tick_dedup_test_")
        self.copy_path = _seeded_copy(self.tmpdir, [
            (self.TARGET_UMR, "test-seed-killed-target-0001", "killed",
             "test-seeded: stuck-task SIGKILL, no exit after grace period"),
        ])
        _insert_row(
            self.copy_path, self.EXISTING_RCA_UMR, "test-seed-existing-rca-0001",
            "completed_unmerged", "real primary deliverable was produced",
            title=f"RCA: {self.TARGET_UMR} killed",
            prompt=f"Real RCA of {self.TARGET_UMR}, already finished, real PR posted.",
        )
        self.state_file = os.path.join(self.tmpdir, "pm-sentinel-inflight.json")
        self.report_file = os.path.join(self.tmpdir, "pm-sentinel-tick-report.jsonl")
        self.metrics_file = os.path.join(self.tmpdir, "pm-sentinel-tick.prom")
        self.env = dict(os.environ)
        self.env["SUPERBOSS_REGISTER_DB"] = self.copy_path
        # Real testability seam (existing convention, see header comment):
        # points the target-identifier-dedup CLI call at THIS checkout's own
        # superboss-register.py (which carries the UMR-20260813-150004
        # extension to PR #297's functions) rather than whatever copy is
        # live-deployed at /opt/veridian/scripts, which this addendum has not
    ...
    def test_killed_row_with_existing_completed_rca_produces_zero_new_rows(self):
        before_rows = _umr_tasks_rows(self.copy_path)
        before_ids = sorted(r["umr_id"] for r in before_rows)

        result = self._run_tick()
        self.assertEqual(result.returncode, 0, msg=result.stdout + result.stderr)
        self.assertIn(f"RCA TARGET DEDUP: rca:{self.TARGET_UMR}", result.stdout)
        self.assertIn(self.EXISTING_RCA_UMR, result.stdout)
        self.assertNotIn(f"DISPATCHING for rca:{self.TARGET_UMR}", result.stdout)
        self.assertIn("0/5 new dispatches this tick", result.stdout)

        after_rows = _umr_tasks_rows(self.copy_path)
        after_ids = sorted(r["umr_id"] for r in after_rows)
        self.assertEqual(before_ids, after_ids, msg="real proof: zero new rows created")

        # DECIDE-AND-FIX still reconciles -- a dedup skip is a real,
        # disclosed disposition of the finding, not a silently dropped one.
        self.assertIn("DECIDE-AND-FIX: 1 real finding(s) this tick, 1 same-tick decision(s)", result.stdout)
        self.assertNotIn("DECIDE-AND-FIX VIOLATION", result.stdout)


class PmSentinelTickRcaRecursionDepthTest(unittest.TestCase):
    """Real regression test for UMR-20260813-150004, item C: real recursion
    incident UMR-20260813-131646-007b ("RCA: UMR-20260813-101802-3ad2
    killed") where 3ad2 was ITSELF "RCA: UMR-20260808-110448-b85c killed" --
    a second-generation RCA, with no depth limit anywhere before this
    addendum. Covers both real behaviors: (1) depth increments and is
    carried into the new dispatch's own prompt when under the limit, and
    (2) dispatch is refused and escalated once at/over MAX_RCA_DEPTH."""

    def setUp(self):
        self.tmpdir = tempfile.mkdtemp(prefix="pm_sentinel_tick_depth_test_")
        self.fake_notify = _fake_notify_owner(self.tmpdir, os.path.join(self.tmpdir, "notify.json"))
        self.notify_log = os.path.join(self.tmpdir, "notify.json")
        self.state_file = os.path.join(self.tmpdir, "pm-sentinel-inflight.json")
        self.report_file = os.path.join(self.tmpdir, "pm-sentinel-tick-report.jsonl")
        self.metrics_file = os.path.join(self.tmpdir, "pm-sentinel-tick.prom")
        self.env = dict(os.environ)
        ...
        self.env["PM_SENTINEL_STATE_FILE"] = self.state_file
        self.env["PM_SENTINEL_MAX_DISPATCH"] = "5"
        self.env["PM_SENTINEL_MAX_RCA_DEPTH"] = "3"
        self.env["PM_SENTINEL_REPORT_FILE"] = self.report_file
        self.env["PM_SENTINEL_METRICS_FILE"] = self.metrics_file
        self.env["PM_SENTINEL_NOTIFY_OWNER_SCRIPT"] = self.fake_notify
        self.env["FAKE_NOTIFY_LOG"] = self.notify_log
    ...
    def test_depth_increments_and_is_carried_when_under_limit(self):
        # A killed row that is ITSELF an RCA task at carried depth 1 (the
        # real 3ad2 shape above) -- the new RCA about to be dispatched for
        # it must carry RCA_DEPTH:2, and must NOT be refused (2 <= 3).
        killed_rca_umr = _gen_umr_id()
        self.copy_path = _seeded_copy(self.tmpdir, [])
        _insert_row(
            self.copy_path, killed_rca_umr, "test-seed-killed-rca-gen1-0001",
            "killed", "test-seeded: this RCA itself got killed",
            title=f"RCA: {_gen_umr_id()} killed",
            prompt="Real RCA prompt, generation 1. RCA_DEPTH:1 (recursion-guard marker).",
        )
        self.env["SUPERBOSS_REGISTER_DB"] = self.copy_path

        result = self._run_tick()
        self.assertEqual(result.returncode, 0, msg=result.stdout + result.stderr)
        self.assertIn(f"DISPATCHING for rca:{killed_rca_umr}", result.stdout)
        self.assertNotIn("RCA RECURSION LIMIT", result.stdout)

        rows = _umr_tasks_rows(self.copy_path)
        new_rows = [r for r in rows if r["umr_id"] != killed_rca_umr]
        self.assertEqual(len(new_rows), 1, msg=rows)
        inputs = json.loads(new_rows[0]["inputs_json"])
        self.assertIn("RCA_DEPTH:2", inputs.get("prompt", ""))

    def test_depth_over_limit_escalates_instead_of_dispatching(self):
        # A killed row that is ITSELF an RCA task already carrying
        # RCA_DEPTH:3 (== MAX_RCA_DEPTH) -- the next generation would be 4,
        # over the limit, so this must escalate via notify-owner.py and
        # dispatch nothing.
        killed_rca_umr = _gen_umr_id()
        self.copy_path = _seeded_copy(self.tmpdir, [])
        _insert_row(
            self.copy_path, killed_rca_umr, "test-seed-killed-rca-gen3-0001",
            "killed", "test-seeded: this RCA itself got killed (generation 3)",
            title=f"RCA: {_gen_umr_id()} killed",
            prompt="Real RCA prompt, generation 3. RCA_DEPTH:3 (recursion-guard marker).",
        )
        self.env["SUPERBOSS_REGISTER_DB"] = self.copy_path
        ...
        result = self._run_tick()
        self.assertEqual(result.returncode, 0, msg=result.stdout + result.stderr)
        self.assertIn(f"RCA RECURSION LIMIT: rca:{killed_rca_umr}", result.stdout)
        self.assertNotIn(f"DISPATCHING for rca:{killed_rca_umr}", result.stdout)
        self.assertIn("0/5 new dispatches this tick", result.stdout)

        after_rows = _umr_tasks_rows(self.copy_path)
        after_ids = sorted(r["umr_id"] for r in after_rows)
        self.assertEqual(before_ids, after_ids, msg="real proof: zero new rows created at the depth limit")

        with open(self.notify_log) as f:
            calls = json.load(f)
        self.assertEqual(len(calls), 1, msg=calls)
        self.assertIn("RCA RECURSION LIMIT REACHED", calls[0]["subject"])
        self.assertIn(killed_rca_umr, calls[0]["body"])
        self.assertIn("depth 4", calls[0]["subject"])
